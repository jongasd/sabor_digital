  ...
  async cadastrarCardapio(dados) {
    ...
    if (!produtos || !Array.isArray(produtos) || produtos.length === 0) {
      throw {
        status: 400,
        mensagem:
          "O cardápio deve conter pelo menos um produto vinculado no formato de array de IDs (ex: [1, 2, 3])",
      };
    }
    ...
    for (const produtoId of produtosUnicos) {
      ...
      if (!produtoExistente) {
        throw {
          status: 404,
          mensagem: `Produto com ID ${produtoId} não encontrado. Cadastro de cardápio cancelado.`,
        };
      }
    }
